...
with open('/etl/Controle_Antonio.csv', 'rt', encoding='utf-8') as file:
	...
	for row in reader :
		try:
			if row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO BA/SE':
				count_BA += 1
			elif row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO CO':
				count_CO += 1
			elif row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO MG':
				count_MG += 1
			elif row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO NE':
				count_NE += 1
			elif row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO PR/SC':
				count_PR += 1
			elif row[5].startswith(('22', '26'))  and row[7] in (today, yesterday) and row[1] == 'CLARO RJ/ES':
				count_ES += 1
		except:
			pass
...
if count == 0:
	message = 'Nenhum site analisado hoje.'
	...
	bot.sendMessage(user_id, str(count)+' checked sites today - Step 22/26. '+OK+'\n\n'+message)
