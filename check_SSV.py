...
with open('/etl/Controle_Antonio.csv', 'rt', encoding='utf-8') as file:
	...
	for row in reader :
		try:
			if row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO BA/SE':
				count_BA += 1
			elif row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO CO':
				count_CO += 1
			elif row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO MG':
				count_MG += 1
			elif row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO NE':
				count_NE += 1
			elif row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO PR/SC':
				count_PR += 1
			elif row[5] == '23 - Aprovar SSV / PFRF' and row[10] == 'ok' and row[7] in (today, yesterday) and row[1] == 'CLARO RJ/ES':
				count_ES += 1
		except:
			pass
...
if count == 0:
	message = 'Nenhuma documentação aprovada hoje.'
	...
	bot.sendMessage(user_id, str(count)+' approved IR/SSVs today. '+OK+'\n\n'+message)
